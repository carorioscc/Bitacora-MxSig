-- Function: getlayertemamat_municipal(geomcoords, character varying, character varying, character varying, numeric, numeric, character varying)

-- DROP FUNCTION getlayertemamat_municipal(geomcoords, character varying, character varying, character varying, numeric, numeric, character varying);

CREATE OR REPLACE FUNCTION getlayertemamat_municipal(
    geomcoords,
    character varying,
    character varying,
    character varying,
...
raise  notice 'entre';
 ...
 --FOR thewhere IN EXECUTE 'select clausulawhen, case when length(trim(filtro))<2 then '''' else ''    '' || filtro end as filtro, tabla from temas.temamat where id = ' || $4 || ';'
 FOR thewhere IN EXECUTE 'select clausulawhen, case when length(trim(filtro))<2 then '''' else ''and '' || filtro end as filtro, tabla,variable from temas.temamat where id = ' || $4 || ';'
 loop
...
--end if;
colores := colores || ' when  '||thewhere.variable||' = ''-8'' then ''-8'''  || ' when  '||thewhere.variable||' = ''-2'' then ''-2'' ' || ' when  '||thewhere.variable||' = ''-3'' then ''-3''' || ' when  '||thewhere.variable||' = ''-99'' then ''-99'' else ''-9'' end)::varchar as color, '; 
-- raise  notice 'colores%', colores; 
 if($1.x2 > $1.x1) then 
...
if (miwhere<>'true') then
 ...
 if (tam>1) then
	...
	if (datos[indice] like '%'||mitabla||'%') then 
		...
		if(indice=1)then
		--nuevowhere := nuevowhere || ' ' || datos[indice];
		end if;
		if(indice>1)then
  ...
  resultado:=6;
        --raise  notice 'nuevowhere %' , nuevowhere;
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION getlayertemamat_municipal(geomcoords, character varying, character varying, character varying, numeric, numeric, character varying)
  OWNER TO postgres;
GRANT EXECUTE ON FUNCTION getlayertemamat_municipal(geomcoords, character varying, character varying, character varying, numeric, numeric, character varying) TO public;
